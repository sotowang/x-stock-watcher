...
$("export").addEventListener("click", () => {
  ...
  const csv=[["Time","Account","Ticker","Type","Direction","Action","Conclusion","Confidence","Original post URL"],...rows.map(({post,signal})=>[post.time||post.capturedAt,`@${post.handle}`,signal.ticker,typeLabels[signal.signal_type]||"Trade",signal.direction==="long"?"Long":"Short",actionLabels[signal.action]||signal.action,signal.conclusion,signal.confidence,post.url])].map(row=>row.map(esc).join(",")).join("\n");
